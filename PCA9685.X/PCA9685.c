...
void SetupPca9685(uint8_t prescale) {
    ...
    I2cWriteByte(PCA9685_ADDR, PCA9685_REG_MODE1, (PCA9685_SLEEP_bm | PCA9685_RESTART_bm | PCA9685_AUTO_INC_bm));
    // Set Prescaler
    I2cWriteByte(PCA9685_ADDR, PCA9685_REG_PRESCALE, prescale);
    // Disable sleep mode
    I2cWriteByte(PCA9685_ADDR, PCA9685_REG_MODE1, PCA9685_AUTO_INC_bm);
    // Configure open-drain, non-inverted, 
    I2cWriteByte(PCA9685_ADDR, PCA9685_REG_MODE2, PCA9685_OUTDRV_bm);
}
...
void SetLed(uint8_t led_number, uint16_t pulse_width_ticks) {
    ...
    I2cWriteBytes(PCA9685_ADDR, led_reg, led_on_off_values, PCA9685_NUM_REG_PER_LED);
}
...
void SetServo(uint8_t servo_number, float position) {
    ...
    SetLed(servo_number, pulse_width_ticks);
}
...
void RunPca9685LightTest() {
    ...
    while (1) {
        for(int i = 0; i < 6; i++) {
            ...
            SetLed(i, pwm_on_duration_per_light[i]);
        }
        _delay_ms(20);
    }
}
